...
def vesicles(vesicle_id):
    ...
    if request.method == "GET":
        app.logger.debug("Processing GET request for <Vesicle [{}]>".format(vesicle_id))
        return get_vesicle_or_error(vesicle_id)

    # For PUT request: store the enclosed vesicle in the database
    elif request.method == "PUT":
        app.logger.debug("Processing PUT request for <Vesicle [{}]>".format(vesicle_id))
        # Validate request
...
def recipient(recipient_id):
    ...
    if len(resp["vesicles"]) > 0:
        app.logger.debug("Returning {count} vesicles for {recipient}\n{filter}\n{listing}".format(
